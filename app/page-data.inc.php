...
Class PageData {
  ...
  static function get_fade($file_path) {
    $fade = array_keys(Helpers::list_files($file_path, '/fade\.(gif|jpg|png|jpeg)$/i', false));
    if (!empty($fade)) {
      $fade = new Image($file_path.'/'.$fade[0]);
      return $fade->data;
    } else {
      return false;
    }
    ...
    static function get_preview_image($file_path) {
    $heroImage = array_keys(Helpers::list_files($file_path, '/preview-image\.(gif|jpg|png|jpeg)$/i', false));
    if (!empty($heroImage)) {
      $hero_image = new Image($file_path.'/'.$previewImage[0]);
  ...
  static function get_logo_image($file_path) {
    $logoImage = array_keys(Helpers::list_files($file_path, '/logo-image\.(gif|jpg|png|jpeg|svg)$/i', false));
    if (!empty($logoImage)) {
      $logo_image = new Image($file_path.'/'.$logoImage[0]);
      return $logo_image->data;
    } else {
      return false;
    }
  }
  ...
  static function create_vars($page) {
    ...
    $page->hero_image = self::get_hero_image($page->file_path);
     # page.fade
    $page->fade = self::get_fade($page->file_path);
    # page.logo_image
    $page->logo_image = self::get_logo_image($page->file_path);
    # page.current_year
